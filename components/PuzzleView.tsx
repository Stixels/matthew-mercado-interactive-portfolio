        ...
        <div className="flex gap-3">
          {target.map((t, i) => (
            <div
              ...
              style={{
                borderColor: values[i] === target[i] ? 'rgba(114,239,221,0.6)' : 'rgba(76,201,240,0.2)',
                background:  values[i] === target[i] ? 'rgba(114,239,221,0.08)' : 'rgba(76,201,240,0.04)',
                color:       values[i] === target[i] ? '#72EFDD' : 'rgba(76,201,240,0.5)',
                boxShadow:   values[i] === target[i] ? '0 0 14px rgba(114,239,221,0.25)' : 'none',
              }}
            >
              {CIPHER_SYMBOLS[t]}
            </div>
            ...
            <div key={i} className="flex flex-col items-center gap-2">
              ...
              <div
                ...
                style={{
                  ...
                  background: matched ? 'rgba(114,239,221,0.06)' : 'rgba(76,201,240,0.04)',
                  boxShadow:  matched ? '0 0 18px rgba(114,239,221,0.25)' : 'none',
                  transition: 'all 0.3s',
                }}
              >
                {/* top fade */}
                ...
                {/* bottom fade */}
                <div className="absolute inset-x-0 bottom-0 h-5 pointer-events-none z-10"
...
const BASE:  Record<TileType, Side[]>       = { corner: ['N','E'], straight: ['N','S'] };
const COLS = 5, ROWS = 3;
...
function floodFill(grid: PipeTile[][]): Set<string> {
  ...
  while (q.length) {
    ...
    for (const side of getConns(grid[r][c])) {
      ...
      if (getConns(grid[nr][nc]).includes(OPP[side])) { vis.add(k); q.push([nc, nr]); }
    }
  }
  return vis;
}
...
function PipeTileCell({
  tile, col, row, litTiles, solved, rotAngle, onClick,
}: {
  tile: PipeTile; col: number; row: number;
  litTiles: Set<string>; solved: boolean;
  rotAngle: number; onClick: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      >
        {tile.type === 'corner' ? (
          <>
            <line x1="29" y1="0"  x2="29" y2="29" stroke={pipeColor} strokeWidth="9" strokeLinecap="round" />
            <line x1="29" y1="29" x2="58" y2="29" stroke={pipeColor} strokeWidth="9" strokeLinecap="round" />
          </>
        ) : (
          <line x1="29" y1="0" x2="29" y2="58" stroke={pipeColor} strokeWidth="9" strokeLinecap="round" />
        )}
      ...
      {/* Entry / exit badge */}
      {isEntry && (
        <span className="absolute top-1 left-1 text-[7px] font-mono leading-none" style={{ color: '#72EFDD', opacity: 0.7 }}>IN</span>
      )}
      {isExit && (
        <span className="absolute top-1 right-1 text-[7px] font-mono leading-none" style={{ color: solved ? '#72EFDD' : 'rgba(157,78,221,0.7)', transition: 'color 0.5s' }}>OUT</span>
      )}
    </div>
  );
}
...
function PipeRouter({ onSuccess }: { onSuccess: () => void }) {
  ...
  const [litTiles, setLitTiles] = useState<Set<string>>(() => floodFill(initState.grid));
  const [solved, setSolved]     = useState(false);
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="overflow-x-hidden" style={{ display: 'grid', gridTemplateColumns: `repeat(${COLS},58px)` }}>
        <div className="flex justify-center">
          <span className="text-[8px] font-mono tracking-widest" style={{ color: '#72EFDD' }}>▶ IN</span>
        </div>
        {Array.from({ length: COLS - 2 }).map((_, i) => <div key={i} />)}
        <div className="flex justify-center">
          <span className="text-[8px] font-mono tracking-widest transition-colors duration-500" style={{ color: solved ? '#72EFDD' : 'rgba(157,78,221,0.7)' }}>OUT ▶</span>
        </div>
      </div>
      ...
      >
        {grid.map((row, ri) =>
          row.map((tile, ci) => (
            <PipeTileCell
              key={`${ci}-${ri}`}
              tile={tile} col={ci} row={ri}
              litTiles={litTiles} solved={solved}
...
function freqDisplay(v: number)  { return (v / 100 * 7 + 1).toFixed(1) + ' Hz'; }
function ampDisplay(v: number)   { return Math.round(v) + '%'; }
function phaseDisplay(v: number) { return Math.round(v * 3.6) + '°'; }
...
function Fader({ value, target, label, sublabel, displayFn, locked, solved, onChange }: FaderProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 select-none">
      {/* Label */}
      <p className="text-[10px] font-mono font-semibold tracking-[0.2em]" style={{ color }}>
        {label}
      </p>
      <p className="text-[8px] font-mono text-zinc-600 tracking-widest">{sublabel}</p>

      {/* Fader track area */}
      <div
        ref={trackRef}
        className="relative touch-none"
        style={{ width: 32, height: TRACK_H, cursor: solved ? 'default' : 'ns-resize' }}
      ...
      >
        ...
        <div
          ...
          style={{ left: 14, top: 0, bottom: 0, width: 4, background: 'rgba(255,255,255,0.06)' }}
        />
        {/* Fill below handle */}
        <div
      ...
      {/* Current value */}
      <p className="text-[11px] font-mono font-bold tabular-nums" style={{ color }}>
        {displayFn(value)}
      </p>
      {/* Target value */}
      ...
      {/* Lock indicator */}
      <p className={`text-[9px] font-mono tracking-widest transition-colors duration-300 ${locked ? 'text-neon-green' : 'text-zinc-700'}`}>
        {locked ? '✓ LOCKED' : '· · · · ·'}
      </p>
    </div>
  );
...
function FrequencyFaders({ onSuccess }: { onSuccess: () => void }) {
  ...
  const checkSolved = useCallback((vals: number[]) => {
    ...
    <div className="flex flex-col items-center gap-5 w-full">
      ...
      <div className="w-full rounded-xl border border-white/[0.06] bg-black/30 p-3">
        <div className="flex justify-between items-center mb-1">
          <p className="text-[9px] font-mono text-zinc-700 tracking-[0.2em]">SIGNAL MONITOR</p>
        ...
        <svg width="100%" viewBox="0 0 240 40" style={{ overflow: 'visible' }}>
          <line x1="0" y1="20" x2="240" y2="20" stroke="rgba(255,255,255,0.04)" strokeWidth="1" />
          <path d={tgtPath} fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1.5" strokeDasharray="4 3" />
          <motion.path
            d={wavePath} fill="none"
            stroke={solved ? '#72EFDD' : '#4CC9F0'} strokeWidth="2"
            animate={{ d: wavePath }}
            transition={{ type: 'spring', bounce: 0, duration: 0.2 }}
            style={{ filter: `drop-shadow(0 0 4px ${solved ? '#72EFDD' : '#4CC9F0'})` }}
          />
        </svg>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <p className="text-[10px] font-mono text-zinc-600 tracking-[0.2em] uppercase text-center">
        {showing ? 'Memorise the sequence…' : solved ? 'Neural Link Established' : 'Repeat the sequence'}
      </p>

      <svg viewBox="0 0 200 185" width={180} height={167} style={{ overflow: 'visible' }}>
        <defs>
          <filter id="hglow">
          ...
          return (
            ...
            >
              ...
              <text
                ...
                style={{ fill: txtFill, fontSize: 18, transition: 'fill 0.2s', pointerEvents: 'none', userSelect: 'none' }}
              >
                {HEX_SYMBOLS[i]}
              </text>
            </g>
        ...
        <div className="flex gap-1.5">
          {sequence.map((_, i) => (
            <div
              ...
              style={{
                background: i < userSeq.length
                  ? (isError ? '#FF4D6D' : '#4CC9F0')
  ...
  if (!activePuzzle) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => router.push('/hub')} className="text-neon-blue font-mono text-sm hover:underline">Return to Hub</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background circuit-grid flex flex-col items-center justify-center relative overflow-hidden pt-[64px] px-6 pb-6 md:px-12 md:pb-12">
          ...
          >
            <div className="text-center">
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 220, damping: 14 }}>
                <CheckCircle size={64} className="mx-auto mb-4" style={{ color: '#72EFDD', filter: 'drop-shadow(0 0 20px rgba(114,239,221,0.8))' }} />
              </motion.div>
              <div className="text-2xl font-bold tracking-widest" style={{ color: '#72EFDD', fontFamily: 'var(--font-orbitron)', textShadow: '0 0 20px rgba(114,239,221,0.6)' }}>
                ACCESS GRANTED
              </div>
              <div className="text-zinc-500 font-mono text-xs mt-2 tracking-widest">Loading classified file…</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        <div className="mb-7">
          ...
          <p className="text-xs font-mono mt-1" style={{ color: `${meta.hex}80` }}>
